...
#
# A test suite for an iterative implementation of a binary search tree with parent pointers.
# There are three methods to test: insert, delete, and search.
#

require 'minitest/autorun'
require_relative 'iterative_binary_search_tree_with_parent_pointers'
...
class TestBinarySearchTree < Minitest::Test
  ...
  def test_when_an_inner_node_with_two_children_is_removed_it_is_replaced_by_the_smaller_child
    ...
    assert smaller_child.parent.equal?(parent)
  end
  ...
  def test_that_branches_are_merged_when_an_inner_node_with_two_children_is_removed
    ...
    assert largest_leaf.parent.equal?(smallest_leaf)
  end
  ...
  def test_that_the_parent_pointer_for_a_new_root_is_nil
    @bst.insert(100)
    node = @bst.insert(200)
    @bst.delete(100)
  ...
  def test_that_branches_are_merged_when_a_root_with_two_children_is_removed
    ...
    assert largest_leaf.parent.equal?(larger_child)
  end
